...
function renderDraw(){
   playerDiv.innerHTML = `<p class="isDraw">DRAW<p>`;
}
...
function checkIfCanWin(){
   ...
   const s1 = [p1, p2, p3]; // Horizontal
   const s2 = [p4, p5, p6]; 
   const s3 = [p7, p8, p9];
   const s4 = [p1, p4, p7]; // Vertical
   const s5 = [p2, p5, p8];
   const s6 = [p3, p6, p9];
   const s7 = [p1, p5, p9]; // Diagonal
   const s8 = [p3, p5, p7];
...
function checkIfWinner(){
   ...
   if (res.length > 0) { // Hay un Ganador
      if (res[0][0] === 'X') {
         playerDiv.innerHTML = `<p class="pcWins">PC WINS<p>`;
         return 'pcwon';
      } else {
         playerDiv.innerHTML = `<p class="userWins">USER WINS<p>`;
         return 'userwon';
      }
   } else { // Si hay Empate
      let draw = true;
...
function renderCurrentPlayer(){
   playerDiv.innerHTML = `<p>${turn === 0 ? 'Player turn' : 'PC turn'}</p>`;
}
